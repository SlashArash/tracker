  ...
  return (
    <div className="w-full max-w-4xl mx-auto py-4 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-white tracking-tight">Projects & Tasks</h2>
          <p className="text-xs text-slate-400">Organize your work streams and task lists</p>
        </div>
      ...
      {/* New Project Form */}
      {isCreatingProject && (
        <form onSubmit={handleAddProject} className="glass-card rounded-2xl p-5 border border-slate-700/60 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-900 border border-slate-700 focus:border-rose-500 text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none"
              autoFocus
            />
          </div>
          ...
          <div className="flex items-center justify-end gap-2 pt-2">
            <button
              ...
              className="px-4 py-2 rounded-xl text-xs font-medium text-slate-400 hover:text-white bg-slate-800/50 hover:bg-slate-800 transition-colors"
            >
              Cancel
            </button>
            <button
      ...
      <div className="space-y-4">
        {projects.length === 0 ? (
          <div className="glass-panel rounded-2xl p-8 text-center text-slate-400 text-sm">
            No projects created yet. Click <span className="text-white font-medium">"New Project"</span> to get started.
          </div>
        ) : (
          projects.map((proj) => {
            ...
            return (
              <div key={proj.id} className="glass-panel rounded-2xl border border-slate-800/80 overflow-hidden transition-all">
                {/* Project Header Bar */}
                <div
                  className="flex items-center justify-between p-4 bg-slate-900/60 cursor-pointer hover:bg-slate-900/80 transition-colors"
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs bg-slate-800 text-slate-400 px-2.5 py-0.5 rounded-full border border-slate-700/50">
                  ...
                  <div className="p-4 bg-slate-950/40 border-t border-slate-800/60 space-y-4">
                    ...
                    <div className="space-y-2">
                      {projectTasks.length === 0 ? (
                        <p className="text-xs text-slate-500 italic py-1">No tasks in this project yet.</p>
                      ) : (
                        projectTasks.map((task) => (
                          <div
                            key={task.id}
                            className="flex items-center justify-between p-2.5 rounded-xl bg-slate-900/40 hover:bg-slate-900/80 border border-slate-800/50 transition-all group"
                          >
                            <button
                    ...
                    <form onSubmit={(e) => handleAddTask(proj.id, e)} className="flex items-center gap-2 pt-2">
                      ...
                      <button
                        type="submit"
                        className="bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white px-3 py-2 rounded-xl text-xs font-semibold flex items-center gap-1 transition-colors cursor-pointer"
                      >
