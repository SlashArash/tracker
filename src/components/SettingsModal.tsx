...
export default function SettingsModal({
	...
	const [workDuration, setWorkDuration] = useState<number | string>(
		settings.workDuration || 25,
	);
	const [shortBreakDuration, setShortBreakDuration] = useState<number | string>(
		settings.shortBreakDuration || 5,
	);
	const [longBreakDuration, setLongBreakDuration] = useState<number | string>(
		settings.longBreakDuration || 15,
	);
	const [longBreakInterval, setLongBreakInterval] = useState<number | string>(
		settings.longBreakInterval || 4,
	);
	const [autoStartBreaks, setAutoStartBreaks] = useState<boolean>(
		settings.autoStartBreaks || false,
	);
	const [soundVolume, setSoundVolume] = useState<number | string>(
		settings.soundVolume ?? 0.7,
	);
	const [soundAlert, setSoundAlert] = useState<SoundAlertOption>(
		settings.soundAlert || "chime",
	);
	const [ambientSound, setAmbientSound] = useState<AmbientSoundOption>(
	...
	return (
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-md p-4">
			<div className="w-full max-w-md glass-card rounded-2xl p-6 border border-border shadow-2xl relative max-h-[90vh] overflow-y-auto text-foreground">
				...
				<form
					onSubmit={handleSave}
					className="py-4 space-y-5 text-xs text-muted-foreground"
				>
					{/* Timer Durations */}
						...
						<div>
							<div className="flex items-center justify-between mb-1">
								...
								<span className="font-mono text-foreground">
									{Math.round(Number(soundVolume) * 100)}%
								</span>
							</div>
							<input
								type="range"
						...
						<div className="grid grid-cols-2 gap-3">
							<div>
								...
								<select
									value={soundAlert}
									onChange={(e) =>
										setSoundAlert(e.target.value as SoundAlertOption)
									}
								...
								>
									...
									<option value="bell" className="bg-card text-card-foreground">
										Resonant Bell
									</option>
									<option value="ping" className="bg-card text-card-foreground">
										Short Ping
									</option>
								</select>
							</div>
							...
							<div>
								...
								<select
									value={ambientSound}
									onChange={(e) =>
										setAmbientSound(e.target.value as AmbientSoundOption)
									}
									className="w-full bg-card border border-border text-foreground rounded-xl px-3 py-2 focus:outline-none"
								>
									<option value="rain" className="bg-card text-card-foreground">
										Gentle Rain (Brown)
									</option>
									<option value="pink" className="bg-card text-card-foreground">
										Pink Noise
									</option>
									<option
										value="white"
