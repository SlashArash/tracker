...
export default function ProjectDashboardGrid({
  ...
  const [editingColorProjectId, setEditingColorProjectId] = useState<string | null>(null);

  // Quick Add Task Form per project grid card
  const [quickTaskTitle, setQuickTaskTitle] = useState<Record<string, string>>({});

  const handleUpdateProjectColor = async (projectId: string, newColor: string) => {
  ...
  const handleDeleteProject = async (projectId: string) => {
    if (!window.confirm('Delete this project and all its associated tasks?')) return;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-base font-bold text-foreground tracking-tight">Project Overview Dashboard</h3>
          <p className="text-xs text-muted-foreground">High-level status, metrics, and progress tracking across projects</p>
        </div>
      ...
      {displayGridProjects.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {displayGridProjects.map((proj) => {
            ...
            const completionRate = projTasks.length > 0 ? Math.round((doneTasks.length / projTasks.length) * 100) : 0;
              ...
              >
                ...
                <div>
                      ...
                      {COLOR_PALETTE.map((c) => (
                        <button
                          ...
                          className={cn(
                            ...
                            proj.color === c ? "scale-110 ring-2 ring-white" : "opacity-70 hover:opacity-100"
                          )}
                          style={{ backgroundColor: c }}
                        >
                          {proj.color === c && <Check className="w-3 h-3 text-white" />}
                        </button>
                      ))}
                    </div>
                  )}
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between text-xs font-semibold">
                      ...
                      <span className="text-foreground font-mono">{completionRate}%</span>
                    </div>
                ...
                <div className="grid grid-cols-3 gap-2 py-2 border-y border-border/50 text-center">
                  <div className="p-2 rounded-xl bg-card border border-border/40">
                    <span className="block text-[10px] text-muted-foreground uppercase font-bold tracking-wider">Done</span>
                  ...
                  <div className="p-2 rounded-xl bg-card border border-border/40">
                    <span className="block text-[10px] text-muted-foreground uppercase font-bold tracking-wider">Active</span>
                  ...
                  <div className="p-2 rounded-xl bg-card border border-border/40">
                    <span className="block text-[10px] text-muted-foreground uppercase font-bold tracking-wider">Logged</span>
                ...
                <form onSubmit={(e) => handleAddQuickTask(proj.id, e)} className="flex items-center gap-1.5">
                  <input
                    type="text"
                    value={quickTaskTitle[proj.id] || ''}
                    onChange={(e) => setQuickTaskTitle({ ...quickTaskTitle, [proj.id]: e.target.value })}
