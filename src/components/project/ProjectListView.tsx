...
export default function ProjectListView({
	...
	const [sortBy, setSortBy] = useState<
		Record<string, "priority" | "status" | "date">
	>({});
	// Visible task limit for pagination per project
	const [visibleListTaskCounts, setVisibleListTaskCounts] = useState<
			...
			{displayProjects.map((proj) => {
				...
				const sortedTasks = [...rawProjectTasks].sort((a, b) => {
					if (currentSort === "priority") {
						...
						return (
							(statusOrder[a.status || "not_started"] || 3) -
							(statusOrder[b.status || "not_started"] || 3)
						);
					} else {
						return (
				...
				const projectDoneCount = rawProjectTasks.filter(
				...
				return (
					<div
						...
						className="glass-panel rounded-2xl border border-border/80 overflow-hidden transition-all shadow-xs"
					>
						{/* Project Header Bar */}
						<div
							className="flex items-center justify-between p-4 bg-card/60 cursor-pointer hover:bg-card/90 transition-colors"
							onClick={() =>
								onSetExpandedProjectId(isExpanded ? null : proj.id)
							}
						>
						...
						{isExpanded && (
							<div className="p-4 bg-background/40 border-t border-border space-y-4">
									...
									{sortedTasks.length === 0 ? (
										...
										(() => {
											...
											const remainingCount =
												sortedTasks.length - displayedTasks.length;

											return (
												<>
													{displayedTasks.map((task) => {
														const statusCfg =
															STATUS_CONFIG[task.status || "not_started"] ||
																...
																<div className="flex items-center gap-2 shrink-0 ml-7 sm:ml-0">
																	...
																	{onStartTaskFocus &&
																		task.status !== "done" && (
																			<button
																				onClick={() =>
																					onStartTaskFocus(proj.id, task.id)
																				}
													...
													{remainingCount > 0 && (
														<button
															onClick={() =>
																setVisibleListTaskCounts({
																	...visibleListTaskCounts,
																	[proj.id]: limit + 5,
																})
															}
								...
								{/* Add Task Input Form */}
								<form
									onSubmit={(e) => onAddTask(proj.id, e)}
									className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 pt-2 border-t border-border/40"
								>
									<input
										type="text"
									...
									<div className="flex items-center gap-2">
										...
										<select
											...
											onChange={(e) =>
												onSetNewTaskPriorities({
													...newTaskPriorities,
													[proj.id]: e.target.value as TaskPriority,
												})
											}
										...
										<select
											...
											onChange={(e) =>
												onSetNewTaskStatuses({
													...newTaskStatuses,
													[proj.id]: e.target.value as TaskStatus,
												})
											}
											className="bg-card border border-border text-foreground rounded-xl px-2.5 py-2 text-xs focus:outline-none cursor-pointer"
