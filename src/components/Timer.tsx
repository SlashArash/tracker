...
import { db, logCompletedSession } from "../services/db";
import type {
	AppSettings,
	Project,
	Task,
...
export default function Timer({
	...
	}, [initialProjectId, setSelectedProjectId, selectedProjectId]);

	useEffect(() => {
		if (initialTaskId !== undefined && initialTaskId !== selectedTaskId) {
			setSelectedTaskId(initialTaskId);
		}
	}, [initialTaskId, setSelectedTaskId, selectedTaskId]);
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: handleTimerComplete uses latest state
	useEffect(() => {
		if (isRunning) {
	...
	const handleStartPause = async () => {
		...
		if (
			newIsRunning &&
			selectedTask &&
			(selectedTask.status === "not_started" || selectedTask.status === "next")
		) {
	...
	const strokeDashoffset =
		strokeDasharray * (1 - Math.min(1, Math.max(0, progressFraction)));
